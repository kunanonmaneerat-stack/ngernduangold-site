...
property: เลือกอัตโนมัติจาก property ที่ verify จริง (prefer โดเมนหลัก ngernduangold.com
          -> sc-domain -> www -> netlify.app เดิม) override ได้ด้วย env GSC_SITE
"""
...
def _resolve_site(svc):
    """เลือก GSC property ที่ถูกต้อง: prefer โดเมนหลัก -> sc-domain -> www -> netlify.app เดิม
    ตามที่ verify จริงใน account (read-only sites.list). ถ้าเช็กไม่ได้ ใช้ SITE ที่ตั้งไว้."""
    prefer = []
    ...
    prefer += [
        ...
        "https://ngernduangold.netlify.app/",
    ]
    try:
        ...
        for cand in prefer:
            if cand in verified:
                return cand
        if verified:
            _log("property ที่ prefer ยังไม่ verify; ใช้ตัวที่มี = %s" % sorted(verified)[0])
            return sorted(verified)[0]
    except Exception as e:
        _log("list sites ไม่ได้ (%s) — ใช้ SITE ที่ตั้งไว้" % e)
    return SITE
