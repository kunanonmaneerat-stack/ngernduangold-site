...
def cta(merchant, url, slug, text):
    ...
    return f'<a class="{cls}" rel="sponsored noopener nofollow" target="_blank" href="{utm(url,merchant,slug)}">{badge}{text}<small>{sub}</small></a>'
...
def cmp_widget(caption, rows, slug, note=""):
    ...
    for r in rows:
        ...
        if r.get("url"):
            cta=f'<a class="go" rel="sponsored noopener nofollow" target="_blank" data-provider="{html.escape(r["name"])}" href="{utm(r["url"],r["camp"],slug)}">เช็ก/สมัคร 👉</a>'
        else:
...
{cta('Kept',KEPT,'kept-savings','โหลดแอป Kept สมัครฟรี 👉')}
...
{cta('Kept',KEPT,'kept-savings','เปิดบัญชีออมเงิน Kept (ฟรี) 👉')}
<h2 id="faq">คำถามที่พบบ่อย</h2>
"""
...
{cta('Krungsri',KRUNGSRI,'easy-approval','สมัครบัตรเครดิต Krungsri ออนไลน์ 👉')}
...
{cta('Krungsri',KRUNGSRI,'easy-approval','เช็กเงื่อนไข + เริ่มสมัครบัตร Krungsri 👉')}
...
{cta('Krungsri',KRUNGSRI,'salary-15000','สมัครบัตรเครดิต Krungsri ออนไลน์ 👉')}
...
<h2 id="which">เงินเดือนเท่านี้ควรเล็งบัตรแบบไหน</h2>
<ul><li>เลือกบัตร “ระดับเริ่มต้น” ที่เกณฑ์รายได้พอดีตัว ไม่เล็งบัตรพรีเมียม</li><li>เน้นบัตรเงินคืน/คะแนนที่ตรงค่าใช้จ่ายประจำ (กิน ช้อป เติมน้ำมัน)</li><li>ดูค่าธรรมเนียมรายปี — หลายใบฟรีปีแรกหรือฟรีเมื่อรูดถึงยอด</li></ul>
...
{cta('Krungsri',KRUNGSRI,'salary-15000','เช็กเงื่อนไข + สมัครบัตร Krungsri 👉')}
<h2 id="faq">คำถามที่พบบ่อย</h2>
"""
...
for _nm,_fit,_pro,_mc,_url,_cn in loan_rows:
    loan_table+=f'<tr><td><b>{_nm}</b></td><td>{_fit}</td><td>{_pro}</td><td><a class="go" rel="sponsored noopener nofollow" target="_blank" href="{utm(_url,_mc,_cn)}">สมัคร \u203a</a></td></tr>'
...
{cta('Srisawad',SRISAWAD,'loan-titleloan','สินเชื่อจำนำทะเบียน ศรีสวัสดิ์ เงินสดทันใจ 👉')}
<p style="text-align:center;color:#5b5b66;font-size:14px">ทางเลือกอื่น: <a rel="sponsored noopener nofollow" target="_blank" href="{utm(CAR4CASH,'Car4Cash','loan-titleloan')}">Car4Cash รถแลกเงิน</a> · <a rel="sponsored noopener nofollow" target="_blank" href="{utm(KTCPBM,'KTCpheboom','loan-titleloan')}">KTC พี่เบิ้ม</a></p>
...
{cta('HappyCash',HAPPYDEBT,'loan-debt','สินเชื่อรวมหนี้ ลดดอก 👉')}
...
{cta('Refinance',REFI,'loan-refi','เช็กรีไฟแนนซ์บ้าน ลดดอก 👉')}
...
{cta('Srisawad',SRISAWAD,'titleloan','จำนำทะเบียน ศรีสวัสดิ์ เงินสดทันใจ รถยังใช้ได้ 👉')}
...
{cta('Srisawad',SRISAWAD,'titleloan','เช็กวงเงินจำนำทะเบียนรถ (ฟรี ไม่มีค่าใช้จ่าย) 👉')}
<p style="text-align:center;color:#5b5b66;font-size:14px">ทางเลือกอื่นเทียบดู: <a rel="sponsored noopener nofollow" target="_blank" href="{utm(CAR4CASH,'Car4Cash','titleloan')}">Car4Cash รถแลกเงิน</a> · <a rel="sponsored noopener nofollow" target="_blank" href="{utm(KTCPBM,'KTCpheboom','titleloan')}">KTC พี่เบิ้ม รถแลกเงิน</a></p>
...
for slug,title,desc,body,faqs,camp in ART:
    ...
    _il=[(s,t) for s,t in [("loan-cash-2026.html","💸 เทียบสินเชื่อทั้งหมด (จำนำทะเบียน/รวมหนี้/รีไฟแนนซ์)"),("links.html","🔗 ลิงก์รวม สมัครบัตร/สินเชื่อ/ออมเงิน"),("","🏠 หน้าแรก เงินเดือนสมองทอง")] if s!=slug]
    ...
    open(f"{OUT}/{slug}","w",encoding="utf-8").write(head(title,desc,slug,ld,og_image=_ogimg)+f'<main class="wrap">{body}{share_bar(slug,_name)}{_nav}</main>'+FOOTER)
...
home_ld=[{"@context":"https://schema.org","@type":"WebSite","name":SITE,"url":BASE+"/","inLanguage":"th"},
 {"@context":"https://schema.org","@type":"Organization","name":SITE,"url":BASE+"/","logo":BASE+"/logo.png","sameAs":["https://www.facebook.com/583765282304956","https://www.threads.net/@non_kuna","https://linktr.ee/kunanonmaneerat"]}]
...
<h2>ติดต่อ</h2><p>สอบถามเพิ่มเติมได้ผ่านช่องทางโซเชียลของเรา</p>"""
...
<h2>ติดต่อเรา</h2>
<p>ติดตามและสอบถามเพิ่มเติมได้ผ่านช่องทางโซเชียลของเรา</p>"""
...
<ul>
<li>รวมทุกช่องทาง: <a href="https://linktr.ee/kunanonmaneerat" target="_blank" rel="noopener">linktr.ee/kunanonmaneerat</a></li>
<li>Facebook: <a href="https://www.facebook.com/583765282304956" target="_blank" rel="noopener">เพจ เงินเดือนสมองทอง</a></li>
<li>Threads: <a href="https://www.threads.net/@non_kuna" target="_blank" rel="noopener">@non_kuna</a></li>
</ul>
...
open(f"{OUT}/contact.html","w",encoding="utf-8").write(head("ติดต่อเรา | "+SITE,"ติดต่อ เงินเดือนสมองทอง ผ่านช่องทางโซเชียล Facebook Threads และ Linktree สอบถาม/เสนอแนะเรื่องการเงินมนุษย์เงินเดือน","contact.html",[])+f'<main class="wrap">{contact_body}</main>'+FOOTER)

# sitemap + robots
urls=[("",("1.0")),("links","0.9"),("about.html","0.4"),("contact.html","0.4"),("disclaimer.html","0.3")]+[(s,"0.8") for s,*_ in ART]
