...
## ⚙️ จูนจากการทดสอบจริง (2026-06-21 — คีย์ GLM+Qwen ใช้ได้แล้ว)
- **POOL จัด qwen นำ** (free_llm.py): qwen-plus → qwen-turbo → glm-4.5-flash → … 
  เพราะ **qwen-plus เขียนสะอาดพร้อมโพสต์/ตามสั่งเป๊ะ** ส่วน glm-4.5-flash ชอบพ่นคำนำ/โครงร่าง (free fallback เมื่อ qwen quota หมด)
- **prompt บีบ 2 ชั้น**: (1) ห้ามคำนำ/โครงร่าง พิมพ์คอนเทนต์จริงเลย (2) **ห้าม hallucinate** ชื่อแบงก์/ผลิตภัณฑ์/เกณฑ์เฉพาะ → พูดภาพรวม + ช่วงกว้าง + ให้ไปเช็กหน้าทางการเอง
- **batch หลายหัวข้อ**: `py pipeline/run_batch.py` (แก้ TOPICS ในไฟล์ได้)

## ⚠️ ข้อจำกัดสำคัญ — ต้อง fact-check ก่อนโพสต์เสมอ
comply_gate จับแค่ **คำต้องห้าม/คำการันตี** — **ไม่จับความถูกต้องข้อเท็จจริง**
ทดสอบเจอ LLM เคยแต่งผิด เช่น "KTC = ของกสิกร" (จริงคือกรุงไทยการ์ด), อ้างแบรนด์ที่เลิกให้บริการแล้ว
→ prompt กันชั้นหนึ่งแล้ว (ให้พูด generic) แต่ **owner/Cowork ต้องตรวจชื่อแบงก์/ตัวเลข/ผลิตภัณฑ์ก่อนโพสต์ทุกครั้ง** = ระบบ draft→human-review by design
