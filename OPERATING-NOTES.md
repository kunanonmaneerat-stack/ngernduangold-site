...
## 31. บั๊กที่แพงที่สุดของระบบนี้ — จับคำโดยไม่ดูบริบท (นับได้ 6 ครั้งใน 1 วัน)

รวมทุกครั้งที่เกิดวันที่ 1 ส.ค. 2026 คนละไฟล์ คนละคนเขียน ตรรกะเดียวกันเป๊ะ:

| # | ที่ไหน | จับผิดว่า | ความจริง |
|---|---|---|---|
| 1 | `check_prompt_drift` | alias `ig` = Instagram | อยู่ในคำว่า `ignore` |
| 2 | `check_prompt_drift` | `2026-08-01` = วันหมดอายุ | เป็นชื่อไฟล์ `HANDOFF_2026-08-01.md` |
| 3 | `check_competing_plan` | `reels/x.mp4` ไม่ใช่คลิปใน reels/ | ต้องการ `/reels/` มีสแลชนำ |
| 4 | สคริปต์ตรวจของผม | `enabled:true` ใน frontmatter | อยู่กลางประโยค `resume = enabled:true` — regex ไม่ anchor |
| 5 | `check_policy_dates_in_prompts` | `until` = คำบอกเส้นตาย | อยู่ในชื่อฟิลด์ `phase_until` |
| 6 | `check_policy_dates_in_prompts` | 19 พรอมป์ตเขียนวันซ้ำ | 13 คือ task ที่ปิดแล้ว · 4 คือ**บรรทัดที่อธิบายบั๊กนี้เอง** · 2 คือตารางหมุนคลัง |

ก่อนหน้านั้นยังมี gate 25 ก.ค. ที่ grep `มีลิงก์พันธมิตร` แล้วไปเจอ `ไม่มีลิงก์พันธมิตร` — **รวมเป็น 7**

### สิ่งที่ทำให้มันกลับมาได้เรื่อยๆ
ทุกครั้งมันมาในรูปแบบใหม่ จึงไม่มีใครจำได้ว่าเป็นอันเดียวกัน · และ **3 ใน 7 อยู่ใน guard ที่เพิ่งเขียนขึ้นมาเพื่อจับ drift**
ครั้งที่ 6 ถึงขั้น**จับบรรทัดที่เขียนสอนว่าอย่าจับคำ** — `agent-auditor` ติดธงเพราะยกคำว่า `Postiz` มาเป็นตัวอย่างในบทเรียนเรื่องการจับคำผิด

### เช็กลิสต์บังคับ ก่อนเขียน matcher ใดๆ (ทุกภาษา ทุกไฟล์)
> 1. **คำสั้น (`ig` `fb` `yt` `until`) ต้องแมตช์ทั้งคำ** — `\b` หรือ lookaround เสมอ
> 2. **anchor ต้นบรรทัดถ้าตั้งใจหาคีย์** — `^enabled:` ไม่ใช่ `enabled:`
> 3. **ตัดชื่อไฟล์ก่อนหาวันที่** — วันในชื่อไฟล์คือการอ้างอิง ไม่ใช่การประกาศ
> 4. **พาธต้องแมตช์ที่ขอบ segment** — `reels/` ตอนต้น หรือ `/reels/` ตรงกลาง
> 5. **คำเดียวกันอยู่ได้ทั้งในคำสั่งและในข้อห้าม** — `ห้ามใช้ X` คือคำตอบ ไม่ใช่ปัญหา
> 6. **บรรทัดที่ยกของเก่ามาห้าม/มาเล่า คือเอกสาร ไม่ใช่ความผิด** — `_ABOUT_DRIFT`
> 7. **task ที่เกษียณแล้ว = บันทึกประวัติ** ไม่ใช่คำสั่งที่ใครจะทำตาม
> 8. **นับ hit แล้วรายงานทันที = สร้างงานปลอมให้คนอื่น** — เปิดอ่านบรรทัดจริงก่อนเสมอ

### และกฎที่ใหญ่กว่าทั้งหมด
> **FAIL/WARN เท็จ อันตรายกว่า guard ที่เงียบ**
> guard เงียบ = ไม่มีใครรู้ · guard ที่ผิดบ่อย = **ทุกคนเรียนรู้ที่จะไม่อ่านมัน** แล้ววันที่มันถูก จะไม่มีใครเห็น
> ตอน `check_policy_dates_in_prompts` ขึ้น 20 ชื่อโดยมีของจริง 1 — นั่นคือ guard ที่กำลังจะฆ่าตัวเองภายในสัปดาห์เดียว


## 32. ปิดเคส "ชั้นที่ 3" ของ GA4 — คำขอออกจริง แต่ Google ตอบ 503 เฉพาะเบราว์เซอร์อัตโนมัติ (1 ส.ค. 2026)

CC พิสูจน์ `buy_intent_click` ได้ 2 ชั้น (listener ทำงาน · `dataLayer` มี event ครบ) แต่ชั้นที่ 3 — *ฮิตออกไปถึง GA4 จริงไหม* — สรุปไม่ได้
เพราะดัก `sendBeacon`/`fetch`/`Image.src` แล้ว **ไม่เห็นการส่งออกเลยแม้แต่ `page_view`** จึงสรุปว่า "เบราว์เซอร์ทดสอบนี้ไม่ส่ง analytics ออก"

**ผมตรวจซ้ำด้วยกลไกคนละตัว — `read_network_requests` ของส่วนขยาย ซึ่งอ่านที่ระดับเบราว์เซอร์ ไม่ใช่แพตช์ JS — ได้ผลต่างออกไป:**

```
POST https://www.google-analytics.com/g/collect?v=2&tid=G-17PPE0M1B8...&en=page_view          → 503
POST .../g/collect?...&en=transport_probe&ep.probe=cowork-chain-check                          → 503
POST .../g/collect?...&_s=3                                                                    → 503
```

### สิ่งที่รู้แน่แล้ว
1. **คำขอออกจริง** — ไม่ใช่ "ไม่ส่ง" · การดัก JS ของ CC พลาดเพราะ GA4 ส่งผ่านช่องที่แพตช์แบบตรงไปตรงมาจับไม่ติด
2. **payload ถูกต้อง** — เห็น `ep.probe=cowork-chain-check` แปลว่า **พารามิเตอร์ custom ถูก serialise ถูกต้อง** → `ep.position` / `ep.product` ของปุ่มซื้อจะไปด้วยแน่นอน
3. **Google ตอบ 503 ทุกครั้งจากเบราว์เซอร์นี้** — ฮิตจากที่นี่ไม่ถูกบันทึก
4. **ผู้ใช้จริงไม่โดน** — GA4 มี 209 sessions + 9 affiliate_click ในช่วงเดียวกัน ถ้าคนจริงโดน 503 ด้วยจะไม่มีข้อมูลเลย

> **สรุป: โค้ด event ถูก · ท่อส่งทำงาน · ปลายทางปฏิเสธเฉพาะเบราว์เซอร์อัตโนมัติตัวนี้**
> ต่างจากข้อสรุปเดิมตรงที่เรา *รู้* แทนที่จะ *อนุมาน* — และผลลัพธ์เชิงปฏิบัติเหมือนกัน

### สองผลพลอยได้ที่สำคัญ
- **โพรบไม่ปนสถิติ** — ตั้งใจใช้ชื่อ event `transport_probe` ที่ไม่มีรายงานไหนอ่าน **ไม่ใช้ `buy_intent_click`** เพราะฮิตปลอมแค่ครั้งเดียวจะเปลี่ยน "0 vs >0" ของ gate 8 ส.ค. ให้อ่านผิดทั้งตาราง · และ 503 ทำให้ไม่มีอะไรเข้าไปอยู่ดี
- **เลิกพยายามยืนยัน GA4 จากเบราว์เซอร์อัตโนมัติได้แล้ว** — มันจะได้ 503 เสมอ ใครลองอีกจะเสียเวลาเปล่า **การยืนยันขั้นสุดท้ายต้องเป็นเจ้าของเปิด GA4 Realtime จากเบราว์เซอร์ปกติเท่านั้น**

> ### กฎที่ได้
> **"ไม่เห็นคำขอ" ไม่เท่ากับ "ไม่มีคำขอ"** — มันแปลว่า*เครื่องมือที่ใช้มองไม่เห็น*
> ก่อนสรุปว่าอะไร "ไม่เกิดขึ้น" ให้ลองด้วยกลไกที่อยู่คนละชั้นเสมอ (แพตช์ JS ↔ network monitor ระดับเบราว์เซอร์)
> เป็นบทเรียนเดียวกับข้อ 25 (Threads OS dialog): **ทดสอบเส้นทาง A แล้วไม่เจอ ≠ ไม่มีอยู่จริง**

---

## 33. GA4 กันทราฟฟิกตัวเองไว้ครบทุกชิ้น แต่กันไม่ได้เลยมาเป็นเดือน — เพราะไอพีถูกหมุน (1 ส.ค. 2026)

ตามหาต้นตอ "79% ของ session เป็น direct แต่ engagement เป็นศูนย์" จนถึงชั้นสุดท้าย แล้วเจอว่า
**ทุกอย่างใน GA4 ถูกตั้งไว้ถูกต้องแล้ว**:

| ชิ้นส่วน | สถานะที่เจอ |
|---|---|
| Internal traffic rule | มีอยู่ ชื่อ `Internal Traffic - My IP` → `traffic_type = internal` |
| Data Filter | `Internal Traffic` · Exclude · **Active** |
| ไอพีที่ปักไว้ | `184.22.17.215/32` |
| ไอพีจริงของเครื่อง (วัดวันนี้) | **`27.130.5.93`** |

ISP หมุนไอพีไปนานแล้ว **กฎจึงไม่ match อะไรเลย** → ไม่มีฮิตไหนได้ `traffic_type=internal`
→ ตัวกรองที่ Active อยู่ก็ไม่มีอะไรให้ตัด → **ทุกครั้งที่ agent เปิดหน้าเว็บถูกนับเป็นคนอ่านจริง**

### ทำไมไม่มีใครจับได้
เพราะ**หน้าจอทุกหน้าบอกว่าปลอดภัย** — เปิด GA4 เข้าไปดูจะเห็นกฎ เห็นคำว่า Active ครบถ้วน
สิ่งเดียวที่ผิดคือตัวเลขในช่อง Value ซึ่งไม่มีอะไรในระบบเคยเทียบกับความจริง
**ไม่มีไฟล์ไหนในเครื่องเคยบันทึกว่าเครื่องนี้ออกเน็ตด้วยไอพีอะไร** จึงไม่มีอะไรที่จะขัดแย้งกับค่านั้นได้

> นี่คือบทเรียนข้อ 1 (ระบบปกติกับระบบตายเงียบหน้าตาเหมือนกัน) ในรูปแบบที่แย่ที่สุด:
> **guard ที่ตั้งค่าครั้งเดียวแล้วไม่มีใครวัดว่ามันยัง match อยู่ไหม = guard ที่ PASS ตลอดโดยไม่ทำงานเลย**
> ต่างจากข้อ 27 ตรงที่คราวนี้ guard ไม่ได้ "เขียนไม่ครบ" — มันเขียนครบ ตั้งถูก แต่**เงื่อนไขโลกภายนอกเปลี่ยน**

### แก้แล้วอย่างไร (ไม่ใช่แค่แก้ไอพี)
1. **ใน GA4** — เพิ่ม `27.130.5.93/32` เป็นเงื่อนไขที่สองของกฎ (เก็บของเดิมไว้ ไม่เสียหาย) แล้ว**เปิดกฎซ้ำเพื่อยืนยันว่าบันทึกจริง**
2. **`tools/uptime_check.py`** — บันทึกไอพีจริงลง `.system_control/host_ip.json` ทุกวัน
   มันคือสิ่งเดียวที่รันจาก Windows Task Scheduler **และ**มีเน็ต จึงเป็นที่เดียวที่สังเกตข้อเท็จจริงนี้ได้
   บันทึกชื่อเครื่องไปด้วย (`host`) เพราะ **sandbox กับเครื่อง Windows ออกเน็ตคนละไอพี** — ถ้ามีใครรันผิดเครื่อง ค่าจะผิดแบบเงียบๆ อีก
   **การบันทึกต้องห้ามกระทบ verdict ของ uptime** (ไอพีล่ม ≠ เว็บล่ม) จึงทำหลังตัดสินผลเสมอ
3. **`preflight → ga4 internal ip`** — เทียบไอพีที่บันทึกกับ CIDR ใน `policy.json → ga4.internal_traffic.ips`
   ไม่ match = WARN ดังพร้อมประโยค "**ตัวเลข GA4 ทั้งหมดเชื่อไม่ได้จนกว่าจะแก้**"
4. **14 เทสต์** รวมเคสจริงของวันนี้ (ปัก 184.22.17.215 · เครื่องอยู่ 27.130.5.93 → ต้อง WARN)
   และเคส `host_ip.json` หาย/เก่าเกิน 7 วัน/อ่านเวลาไม่ออก → **WARN เสมอ ห้าม PASS**
   *ตาบอดต้องไม่มีวันพิมพ์ PASS* — ถ้าตรวจไม่ได้ต้องบอกว่าตรวจไม่ได้

### ทำไมเป็น WARN ไม่ใช่ FAIL
FAIL ในระบบนี้แปลว่า "ห้ามโพสต์" ไอพีเพี้ยนไม่ได้ทำให้โพสต์ผิดหรือเว็บพัง
แต่มันทำให้**ตัวเลขทุกตัวที่ใช้ตัดสินใจเชื่อไม่ได้** — จึงต้องดังพอให้ watchdog ยกขึ้นรายงาน แต่ไม่บล็อกงานที่ยังถูกต้อง

### กับดักที่เกือบพลาดซ้ำ
อยากขยายเป็น `27.130.5.0/24` ให้ทนการหมุนไอพี — **อย่า** /24 ของ ISP ไทยมีคนจริงอยู่ได้
การตัดคนอ่านจริงทิ้งแย่กว่าการนับฮิตตัวเองปนเข้ามาไม่กี่ครั้ง **เลือกให้แคบไว้ แล้วทำให้ "แคบจนหลุด" ดังแทน**

> ### กฎที่ได้
> **การตั้งค่าที่อ้างอิงสิ่งที่โลกภายนอกเปลี่ยนได้ (ไอพี · โทเคน · โควตา · URL) ต้องมีตัววัดว่ามันยังตรงอยู่**
> ไม่งั้นมันจะไม่พังตอนเปลี่ยน — **มันจะยังดูดีต่อไปเรื่อยๆ แล้วเงียบ**

### 33.1 ต่อจากข้อ 33 — สองแหล่ง ไม่ใช่แหล่งเดียว และการแก้คนละตัวกัน (1 ส.ค. 2026 ค่ำ · Cowork + CC)

CC วัดด้วย Playwright แล้วพบว่า clicktest **ส่ง hit ถึง GA4 จริง** (`POST /g/collect?tid=…`)
แต่ UA เป็น `HeadlessChrome` + `navigator.webdriver=true` → **GA4 กรอง known bots ให้เอง**
จึงอธิบายช่องว่าง "ควรมี ~21 คลิก แต่เห็น 9" ได้พอดี

**ข้อสรุปที่ตามมาและไม่มีใครเขียนไว้ (สำคัญกว่าตัวเลข):**

| แหล่งทราฟฟิกของเราเอง | GA4 มองเป็นบอทไหม | อะไรที่กันมันได้ | สถานะก่อน 1 ส.ค. |
|---|---|---|---|
| clicktest (Playwright headless) | **ใช่** → ถูกกรองเกือบหมด | opt-out ที่ต้นทาง | ไม่มี → หลุดบางส่วน |
| เบราว์เซอร์ Chrome จริงของ agent | **ไม่ใช่** → ไม่ถูกกรองเลย | **กฎ internal traffic (ไอพี) เท่านั้น** | กฎตายเพราะไอพีหมุน |

⇒ **79% ที่เป็น direct ส่วนใหญ่ไม่ใช่ clicktest** — clicktest ถูก Google กรองไปเกือบหมดอยู่แล้ว
ตัวที่รอดเข้ามาเป็นก้อนใหญ่คือ**เบราว์เซอร์จริง** ซึ่งมีแค่กฎไอพีเท่านั้นที่กันได้ และกฎนั้นตายอยู่
**การแก้สองอันจึงกันคนละกลุ่มกัน ไม่มีอันไหนแทนกันได้** ใครอ่านย้อนหลังจะเดาว่าอันเดียวพอ — ไม่พอ

**สิ่งที่เพิ่มเข้าไปอีกชั้น:** CC พิสูจน์ว่า opt-out ได้ผลด้วยมือครั้งเดียว (1 คำขอ → 0)
**การพิสูจน์ครั้งเดียวจะเน่าทันทีที่มีคนแก้ `WRAP` หรือเพิ่มฟังก์ชัน `run_*` ตัวที่สี่แล้วลืมเรียก**
และมันจะเน่าแบบเงียบสนิท เพราะ clicktest ตรวจผลจาก `dataLayer` ไม่ได้ตรวจ network —
**ยิง GA4 อยู่หรือไม่ยิง มันก็พิมพ์ 11/11 PASS เหมือนกัน**
จึงให้ตัวมันนับคำขอที่ออกไปหา `*/collect` เอง แล้ว **FAIL ถ้าไม่เป็นศูนย์** และรวมการตั้งค่าหน้าไว้ที่ `_arm()` จุดเดียว (เดิมก็อปไว้ 3 จุด = บั๊กคลาส "ข้อเท็จจริงเดียวหลายที่")

**reverse test (บังคับตามกฎของวันนี้):** opt-out เปิด → **0 คำขอ** · ถอด `ga-disable` ออกบรรทัดเดียว → **2 คำขอ**
ตัวนับไม่ได้เงียบเพราะตาบอด มันเงียบเพราะไม่มีอะไรหลุดจริง

> ### กฎที่ได้
> **"พิสูจน์แล้วด้วยมือ" มีอายุเท่ากับการแก้ไฟล์ครั้งถัดไป** — ถ้าสิ่งที่พิสูจน์สำคัญพอจะพิสูจน์
> มันก็สำคัญพอที่จะให้เครื่องพิสูจน์ซ้ำทุกครั้ง **โดยเฉพาะเมื่อสัญญาณที่มนุษย์เห็น (11/11 PASS) เหมือนกันทั้งตอนถูกและตอนพัง**

---

## 34. โฟลเดอร์ที่มีอยู่ ≠ งานที่มีชีวิต — "ไฟล์ล่อ" 10 ตัวที่ guard มองไม่เห็น (7 ส.ค. 2026)

ก่อนจะเขียนคำสั่งให้ CC แก้ `ngernduangold-buy-intent-gate` ผมไปเช็กก่อนว่าไฟล์อยู่ที่ไหนจริง แล้วเจอว่า:

```
C:\Users\nL_ku\.claude\scheduled-tasks\ngernduangold-buy-intent-gate\SKILL.md   5,549 bytes
C:\Users\nL_ku\Claude\Scheduled\ngernduangold-buy-intent-gate\SKILL.md          5,549 bytes
hash ตรงกัน · mtime เดียวกัน (1 ส.ค. 19:00)
```

**แต่ `list_scheduled_tasks` ไม่มี id นี้เลย** ⇒ ไฟล์ฝั่ง Cowork **ไม่มีใครยิง** และ**การไปแก้มันไม่มีผลอะไรทั้งสิ้น**

### กวาดทั้งระบบแล้วเจอ 10 ตัว

| | จำนวน |
|---|---|
| โฟลเดอร์ใน Cowork root (ไม่นับ `_shared`) | 100 |
| id ใน registry (`list_scheduled_tasks`) | 90 |
| โฟลเดอร์ใน CC root | 12 |
| **กำพร้า (มีโฟลเดอร์ ไม่มี registry)** | **10** |
| ผี (มี registry ไม่มีโฟลเดอร์) | 0 |

- **9 ตัวเป็นสำเนาล่อ** ของงานฝั่ง CC → ตัวจริงอยู่ CC root
- **`tiktok-vmok-window-watcher` ไม่อยู่ทั้ง registry และ CC root** ⇒ **ไม่มีใครยิงเลยทั้งระบบ**
  และพรอมป์ต `agent-auditor` เขียนไว้ว่ามันเป็น "งานฝั่ง CC ที่ยังมีชีวิต" — **ตัวตรวจสอบถือรายชื่อที่ผิดมาตลอด**

### ทำไม guard ที่มีอยู่จับไม่ได้
`preflight → check_task_mirror` เทียบ **เนื้อไฟล์** ของสอง root
ไฟล์เหมือนกันทุกไบต์ → **PASS**

> **guard พิสูจน์ได้แค่ว่า "ไฟล์ตรงกัน" ไม่ได้พิสูจน์ว่า "ทั้งสองฝั่งยังมีชีวิต"**
> ไฟล์กำพร้าที่ไม่มีใครยิง **หน้าตาเหมือนงานที่ mirror ถูกต้องทุกประการ**

ต่างจากข้อ 29/30 ตรงที่ครั้งนั้นคือ *สองสำเนาไม่ตรงกัน* ครั้งนี้คือ **สองสำเนาตรงกันเป๊ะ แต่ฝั่งหนึ่งเป็นของปลอม**
และเคส weekly-review เมื่อ 1 ส.ค. ก็เกิดจากความสับสนเรื่อง "root ไหนเป็นเจ้าของอะไร" อันเดียวกันนี้

### แก้ที่ไหน — และทำไมไม่ใช่ preflight
**registry ไม่ใช่ไฟล์ในทั้งสอง root** มันอยู่ใน store ของแอป เรียกได้ผ่าน MCP เท่านั้น
`preflight` รัน offline ไม่มี MCP → **เขียน check นี้ไม่ได้จริง**

> **การเขียน check ที่อ่านของจริงไม่ได้ แย่กว่าการไม่มี check เลย เพราะมันจะพิมพ์ PASS ตลอดไป**
> — ซึ่งคือความล้มเหลวที่ทั้งการตรวจรอบนี้พยายามกำจัด

จึงย้ายไปอยู่ที่ **`cowork-task-watchdog` ข้อ 2.6** ซึ่งมีทั้ง `list_scheduled_tasks` และ shell
บันทึกผลกวาดไว้ที่ `policy.json → task_roots` ให้ watchdog เทียบทุกเช้า — ตัวเลขเปลี่ยน = มีอะไรเกิดขึ้น

### ไม่ลบอะไรเลย
โฟลเดอร์กำพร้าอยู่ใน root ของ Cowork แต่ตัวจริงเป็นของ CC — **การลบไฟล์ของอีก agent คือสิ่งที่ทำให้เกิดเคส weekly-review**
รายงานและบันทึกไว้ พอแล้ว

> ### กฎที่ได้
> **"มีไฟล์อยู่" ไม่ใช่หลักฐานว่า "มันทำงาน"** — ก่อนแก้พรอมป์ตของงานใด ให้ยืนยันว่า id นั้นอยู่ใน `list_scheduled_tasks` จริง
> ถ้าไม่อยู่ แปลว่ากำลังจะแก้ตัวล่อ **และจะเดินจากไปโดยเชื่อว่าแก้เสร็จแล้ว**

---

## 35. ตัวเฝ้าระวังหลักตายมา 12 วัน และสัญญาณเดียวที่ทุกคนดูคือสัญญาณที่โกหก (7 ส.ค. 2026)

เจ้าของถามว่า "ทำไม Slack ไม่อัพเดต" คำตอบที่ทุกคนเดา (แอปปิด) **ผิด**

### หลักฐาน
`cowork-task-watchdog` · `daily-social-post-reminder` · `post-guard-daily` · `channel-heartbeat`
**มี `lastRunAt` วันที่ 6 ส.ค. ครบทั้งสี่ตัว** และมีคอนเทนต์ขึ้นจริงวันนั้นด้วย
แต่ข้อความสุดท้ายใน Slack คือ **2 ส.ค.** และไม่มีไฟล์ใดถูกเขียนใต้ `~\Claude` เลยช่วง 6 ส.ค. 07:50–09:30

ไล่ต่อที่ไฟล์ log ของ watchdog เทียบกับ slot 08:07:

| ไฟล์ | เวลาที่เขียน | |
|---|---|---|
| 26 ก.ค. | 08:12 | **ตรง slot — รันตามตารางครั้งสุดท้าย** |
| 30 ก.ค. | 15:59 | นอก slot = รันมือ |
| 31 ก.ค. | 15:51 | นอก slot = รันมือ |
| 1 ส.ค. | 12:59 | นอก slot = รันมือ |
| 2–7 ส.ค. | — | ไม่มีเลย |

⇒ **การรันตามตารางของ watchdog ไม่เคยสำเร็จอีกเลยตั้งแต่ 26 ก.ค. — 12 วัน**
(26 ก.ค. คือวันที่ 16 งานถูกปิดยกชุดรวมทั้งตัวมันเอง · เปิดคืน 31 ก.ค. · **หลังเปิดคืนไม่เคยจบสักรอบ**)

### ทำไมไม่มีใครรู้
`lastRunAt` เดินหน้าทุกเช้าตลอด 12 วัน ทุกหน้าจอจึงบอกว่าปกติ

> **`lastRunAt` พิสูจน์แค่ว่างาน *เริ่ม* ไม่ได้พิสูจน์ว่ามัน *จบ***
> งานที่ค้างรอ permission prompt หรือหลุด connector กลางทาง ก็ประทับ `lastRunAt` เหมือนกันเป๊ะ

และสาเหตุที่น่าจะเป็นคือ **การแก้พรอมป์ตทำให้ tool approval ที่เก็บไว้กับ task ใช้ไม่ได้**
พรอมป์ต `agent-auditor` เขียนเตือนเรื่องนี้ไว้เองด้วยซ้ำ ("approval-trap · แนะ Run now")
**แปลว่าการลงมือ *ซ่อม* งาน สามารถทำให้งานนั้นหยุดเงียบได้** — และตัวผมเองก็เพิ่งแก้พรอมป์ตนี้ไป 3 ครั้งวันนี้

### สิ่งที่แย่ที่สุดของเคสนี้
ตัวที่ตายคือ **ตัวที่มีหน้าที่เฝ้าตัวอื่นทั้งหมด** — ทุก guard ที่เราสร้างมาทั้งเดือนรายงานผ่าน watchdog
**guard ที่เฝ้า guard ตายเงียบ = guard ทั้งกองไม่มีใครฟัง**

### แก้อย่างไร
1. **ตรวจจับได้แล้ว** — `agent_gap_check.py` เพิ่มแกน `reporting` ยิง `REPORTING-DARK` เมื่อ*งานสดแต่ไม่มีรายงานลงวันที่เกิน 48 ชม.* รันจริงครั้งแรกจับได้ทันที
2. **ปลดล็อกต้องเป็นเจ้าของ** — กด **Run now** หนึ่งครั้งเพื่อ pre-approve tool ให้ task
3. **กฎใหม่:** ทุกครั้งที่แก้พรอมป์ตของ scheduled task **ต้องกด Run now หนึ่งรอบเสมอ** แล้วยืนยันว่ามันเขียนผลงานออกมาจริง

> ### กฎที่ได้
> **อย่าวัดว่างานทำงานอยู่จากสัญญาณที่งานนั้นปล่อยตอน *เริ่ม* — วัดจาก *ผลงานที่มันควรผลิตออกมา***
> ถ้างานควรเขียนไฟล์ ให้ตรวจว่าไฟล์มา · ถ้าควรโพสต์ ให้ตรวจว่าโพสต์ขึ้น
> **`lastRunAt`, PID, "ไม่มี error" — ทั้งหมดนี้คือสัญญาณของการเริ่ม ไม่ใช่ของการสำเร็จ**

### 35.1 ต้นตอจริงของข้อ 35 — ไม่ใช่ approval-trap แต่เป็น "ลำดับขั้นตอนผิด" (7 ส.ค. 2026 · ต่อจากข้อ 35)

ข้อ 35 เดาว่าเป็น approval-trap **เดาผิด** พอนับผลงานจริงของวันที่ 6 ส.ค. ได้ภาพชัด:

**วันนั้น agent ผลิตผลงานออกมา 14 ชิ้น** — โพสต์ · guard rows · routine events
งานที่**จบสมบูรณ์**: `video-post-verify` · `knowledge-post-noon` · `threads-video-idle-window` · `yt-comment-link` · `channel-heartbeat`
งานที่**ไม่ผลิตอะไรเลย**: `cowork-task-watchdog` · `daily-social-post-reminder`

> **จุดร่วมเดียวของสองตัวที่ล้ม: ทั้งคู่มี "โพสต์ Slack" เป็นงานหลัก**
> **ห้าตัวที่รอด ไม่มีตัวไหนต้องแตะ Slack เลย**

ไม่ใช่ระบบ approval พังทั้งระบบ — **Slack ตัวเดียวที่ล้ม** และมันลากรอบทั้งรอบตายไปด้วย

### ทำไมความล้มเหลวของ Slack ถึงลบหลักฐานทิ้งไปด้วย
พรอมป์ตเดิมเรียงว่า **ขั้น 8 = โพสต์ Slack · ขั้น 9 = เขียน log**
พอ Slack ล้ม รอบนั้นตายก่อนถึงขั้น 9 → **ไม่มีทั้งข้อความ Slack และไม่มีทั้งบันทึก**
ไม่เหลืออะไรให้รู้เลยว่าเคยรัน นอกจาก `lastRunAt` ที่บอกแค่ว่า "เริ่มแล้ว"

ซ้ำร้าย ขั้น 9 เขียนไว้เองว่า *"skip silently if the write fails"*
— **การออกแบบให้ล้มเหลวเงียบ ในไฟล์ที่มีหน้าที่จับการล้มเหลวเงียบ**

### แก้แล้ว — สลับลำดับ + กันไม่ให้ล้มทั้งรอบ
| เดิม | ใหม่ |
|---|---|
| 8 · โพสต์ Slack | **8 · เขียน log + ยืนยันด้วย `Test-Path`** |
| 9 · เขียน log (ข้ามเงียบถ้าล้ม) | **9 · โพสต์ Slack — ครอบด้วยการดักล้มเหลว ห้ามหยุดรอบ** |

Slack ล้ม = บันทึกว่าล้ม แล้วไปต่อ พร้อมขึ้นบรรทัดแรกของรายงานว่า
`⚠️ โพสต์ Slack ไม่สำเร็จ — <error> · log เขียนไว้แล้วที่ <path>`

> ### กฎที่ได้
> **บันทึกหลักฐานต้องถูกเขียนก่อนการเรียกบริการภายนอกที่ล้มได้เสมอ**
> ไม่งั้นเมื่อบริการนั้นล้ม จะเสียทั้งงานและเสียทั้งหลักฐานว่าเคยพยายาม — และการวินิจฉัยครั้งถัดไปจะเริ่มจากศูนย์
> เป็นเหตุผลเดียวกับที่ `uptime_check` บันทึกไอพี**หลัง**ตัดสิน verdict: ขั้นตอนที่ล้มได้ ห้ามอยู่ต้นน้ำของขั้นตอนที่ต้องรอด

## 36. "เสบียงพอไหม" ตอบผิดได้ 3 ทางซ้อน (7 ส.ค. 2026)

เจ้าของถามสั้นๆ ว่าคอนเทนต์พอไหม การจะตอบให้ถูกต้องเจอกับดัก 3 ชั้น

### ชั้น 1 — join key ผิด แล้วได้คำตอบที่ดูสมเหตุสมผล (บั๊กคลาสข้อ 31 ครั้งที่ 9)
นับ "คลิปที่ยังไม่เคยโพสต์" โดย join `manifest.reel` → `ledger.clip_id` ได้ **9 ตัว** ดูเหมือนมีของเหลือเฟือ
ความจริง: แถว ledger ของคลิปเดือน ก.ค. **ไม่มีฟิลด์ `clip_id`** (ยุคนั้นใช้ `text_hash`) — 3 ใน 4 แถวว่าง
`manifest.posted` ซึ่งเป็นความจริงอีกแหล่งบอกว่า 7 ใน 9 ตัวลงไปแล้ว — **เหลือจริง 2**
ถ้ารายงานไปเลยจะบอกเจ้าของว่า "มีของ 9 วัน" ทั้งที่มี 2 แล้วช่องจะเงียบโดยไม่มีใครรู้ตัว

> **กฎ: ก่อนสรุปจากการ join ต้องพิสูจน์ก่อนว่าคีย์นั้นมีค่าจริงในข้อมูลทั้งสองฝั่ง**
> "join แล้วได้ 0" กับ "join ด้วยคีย์ที่ฝั่งหนึ่งไม่มี" ให้ผลหน้าตาเหมือนกันสนิท

### ชั้น 2 — "มีไฟล์" ไม่เท่ากับ "ใช้ได้"
2 ตัวที่เหลือ (+ อีก 4 ที่ลงไปบางช่อง) เป็น **720x1280** ทั้งหมด · สเปคบังคับคือ **1080x1920**
`check_queued_clip_spec` จะปัดตกทันทีถ้าเอาเข้าคิว → มันไม่ใช่ runway มันคือไฟล์ที่บังเอิญมีอยู่
บันทึกไว้ที่ `policy.json -> content_supply.video.clips_720p_unusable` แล้ว จะได้ไม่มีใครนับซ้ำ

> **กฎ: นับเสบียงต้องนับผ่านเกณฑ์ที่ประตูทางเข้าใช้จริง ไม่ใช่นับจำนวนไฟล์**

### ชั้น 3 — คิวว่างไม่ได้แปลว่าพัง
9-10 ส.ค. คิวว่างจริง แต่เป็นการ **จอดตามมติ** `GATE-batch3_20260806.md` เพื่อไม่ผลิตทิ้งก่อน gate 10 ส.ค.
เดิม `runway_guard` แยกไม่ออกระหว่าง "จอดตามแผน" กับ "ท่อขาดเงียบแบบ 27-30 ก.ค." — ตะโกนเหมือนกันทั้งคู่
ซึ่งอันตรายทั้งสองทาง: ตะโกนทุกวันที่จอด = สอนให้เจ้าของเมินตอนที่มันดังจริง

เพิ่ม `content_supply.planned_park` + verdict `PARKED` / `PARK_OVERRUN`
- อยู่ในหน้าต่าง -> `PARKED` exit 0 พิมพ์ว่าจอดถึงวันไหน ใครสั่ง อ้างเอกสารอะไร
- **เลยวันหมดอายุแล้วคิวยังสั้น -> `PARK_OVERRUN` ดังกว่า LOW_RUNWAY ธรรมดา** เพราะแผนพัง ไม่ใช่แค่คิวหมด
- park ที่ไม่มี `until` หรือ parse ไม่ได้ -> **ปฏิเสธ ไม่ใช่ยอมรับ**
- park **ไม่เคย** กลบ `DRIFT` — สามไฟล์ไม่ตรงกันคือท่อพัง ไม่มีมติไหนทำให้ยอมรับได้

ทดสอบย้อนทางด้วยวันจริง 4 วัน: 7 ส.ค. = LOW_RUNWAY · 9-10 ส.ค. = PARKED · **11 ส.ค. = PARK_OVERRUN**

> **กฎ: guard ที่ยอมให้มี "ข้อยกเว้น" ต้องบังคับวันหมดอายุของข้อยกเว้นนั้นเอง**
> ไม่งั้นข้อยกเว้นคือสวิตช์ปิด guard ที่แต่งตัวมาดี — เป็นบั๊กคลาส "กฎถาวรที่เขียนเป็นวันตายตัว" ในข้อ 33 กลับด้าน

### สิ่งที่ทำต่อโดยไม่ต้องรอสั่ง
gate เขียนไว้เองว่า *"คอขวดคือสคริปต์/เสียง"* -> เขียน `BATCH4-PRODUCTION-KIT.md` 5 คลิปล่วงหน้า
เป็นข้อความล้วน ไม่ผลิตเสียง/วิดีโอ = **ถ้า gate 10 ส.ค. สั่งลด cadence ก็ทิ้งได้ ไม่มีต้นทุนจม**
เลือกหัวข้อจากตัวเลข: cluster ของ b3-05 ที่ทำ 304 วิว + หน้าที่ GSC ยืนยันว่าติดหน้า 1 แล้ว (pos 1, 4, 6)
ไม่ใช่ cluster รถที่ impression เยอะกว่าแต่อันดับ 47-88

## 37. รอบตรวจซ้ำ 8 ส.ค. 2026 — ของที่เพิ่งซ่อมสร้างรูใหม่สองรู

วนตรวจใหม่ทั้งระบบหลังแก้เรื่องเสบียง เจอว่า **การแก้เมื่อวานเองสร้างปัญหาใหม่ 2 อย่าง**
ทั้งคู่เป็นบั๊กคลาสเดียวกัน: **เพิ่มค่าใหม่ที่ฝั่งผู้ผลิต แล้วลืมบอกฝั่งผู้บริโภค**

### 37.1 เพิ่ม verdict ใหม่ให้ guard แล้วลืมว่ามีคนอ่านมันอยู่
`runway_guard` ได้ verdict `PARKED` / `PARK_OVERRUN` เมื่อวาน
แต่ `preflight.check_queue` เขียนไว้ว่า `OK → PASS · LOW_RUNWAY → WARN · else → FAIL`
⇒ **9 ส.ค. ที่คิวจอดตามแผน preflight จะ FAIL พร้อมข้อความว่างเปล่า** (เพราะ `problems` ว่าง)
FAIL ที่ไม่มีข้อความบอกอะไรไม่ได้เลย และจะเกิดในวันที่ระบบทำงานถูกต้องที่สุด

แก้: map ครบทุก verdict + **สาขา unknown ต้องบอกชื่อ verdict ที่มันไม่รู้จัก**
เพื่อให้คนที่เพิ่ม verdict ตัวถัดไปได้ยินเสียง ไม่ใช่ได้ FAIL เปล่าๆ
เพิ่ม `run_check_detail()` ในชุดทดสอบด้วย — เดิม assert ได้แค่ status
**status ถูกแต่ข้อความว่าง = ผ่านเทสต์แต่ไร้ประโยชน์** ซึ่งเป็นสิ่งที่เกิดขึ้นจริง

### 37.2 ป้ายหลุมศพถูกนับเป็น drift ทุกวัน
`ngernduangold-weekly-review` เป็น id เดียวที่เคยหมายถึงงานคนละงานในสอง scheduler
ฝั่ง CC ถูกแทนด้วยป้ายหลุมศพโดยตั้งใจ ⇒ **สองไฟล์ต้องต่างกัน**
แต่ `check_task_mirror` เทียบเนื้อไฟล์ตรงๆ จึงเตือนทุกวันเรื่องที่แก้ไปแล้ว
**การเตือนเรื่องที่แก้แล้วซ้ำๆ = สอนให้คนอ่านข้ามด่านนี้** ซึ่งคือสาเหตุที่ drift จริงของ `pantip-monitor` ไม่มีใครเห็นอยู่วันหนึ่ง

แก้: ใช้ `_RETIRED` ที่ด่านอื่นใช้อยู่แล้ว — **ฝั่งเดียวเป็นหลุมศพ = collision ที่แก้แล้ว**
แต่กันช่องโหว่ไว้: **สองฝั่งยัง live แล้วต่างกัน = drift เหมือนเดิม** และยังพิมพ์บอกว่ามีหลุมศพอยู่กี่ตัว
ห้ามเงียบสนิท ไม่งั้นวันที่มีคนเขียนทับหลุมศพกลับเป็นงานจริงจะไม่มีใครรู้

## 38. เขียนหลักฐานก่อน แล้วต้องมีคนอ่านหลักฐานนั้นด้วย (8 ส.ค. 2026)

เมื่อวานแก้ 5 พรอมป์ตให้ **เขียนหลักฐานก่อนเรียกของที่ล้มได้**
`log_run.py` เก็บแถวล่าสุดต่อ routine ⇒ รอบที่ตายกลางทางจะทิ้ง `status: started` ค้างไว้ตลอดกาล

**แต่ไม่มีใครอ่านมัน** — ตารางนั้นดูปกติทุกประการสำหรับคนที่ไม่ได้ diff
⇒ เพิ่ม `check_stuck_runs` ใน preflight: routine ที่แถวล่าสุดเป็น `started` และเก่ากว่า 12 ชม. = เริ่มแล้วไม่จบ
- **WARN ไม่ใช่ FAIL** เพราะ preflight รัน 08:00 พร้อมงานอื่นหลายตัว — กำลังทำอยู่จริงต้องไม่โดนหาว่าค้าง
- `started` แล้วตามด้วย `ok` **หรือ** `fail` = จบแล้วทั้งคู่ (รายงานว่าล้มก็คือรายงาน)
- แถวที่ไม่มี key `routine` (เช่น post-ledger ที่อยู่โฟลเดอร์เดียวกัน) ต้องข้าม ไม่ใช่ crash
- ts อ่านไม่ออก ต้องโผล่ ไม่ใช่ถูกข้ามเงียบ

> **กฎ: ทุกครั้งที่เพิ่มหลักฐานชนิดใหม่ ต้องเพิ่มคนอ่านหลักฐานนั้นในรอบเดียวกัน**
> ไม่งั้นสิ่งที่ได้คือ log ที่ดูดีขึ้น กับความมั่นใจที่ไม่มีอะไรรองรับ

## 39. เครื่องมือสองตัวที่ควรมีตั้งนานแล้ว (8 ส.ค. 2026)

- **`tools/encoding_probe.py`** — สแกนไทยเพี้ยน (U+FFFD) ทั้งสอง task root + รีโป
  แยก 3 กรณีที่หน้าตาคล้ายกันแต่ต้องทำคนละอย่าง: **เสียแล้วจริง** · **BOM (เนื้อหาไม่เสีย)** · **cp874 ที่ยังกู้ได้ (ห้าม re-save ทับ)**
  รันครั้งแรก: 1,695 ไฟล์ · เสีย 0 · BOM 3
  ใช้พิสูจน์ทันทีว่าตัว U+FFFD ที่โผล่ในผล diff เป็น artifact ตอนพิมพ์ผ่าน console **ไม่ใช่ไฟล์เสีย** — เดิมจะต้องเดา
- **`tools/mirror_diff.py`** — บอกว่า id ไหน *ต่างกันตรงไหน* ระหว่างสอง task root
  เดิม preflight บอกได้แค่ "สองอันนี้ไม่ตรงกัน" ซึ่งพอให้รู้ว่ามีปัญหา แต่แก้ไม่ได้
  และการเขียน diff เองบน Windows ต้องไปสู้กับ quoting ของ PowerShell ก่อนทุกครั้ง

> **บทเรียนร่วม:** ด่านที่บอกว่า "มีบางอย่างผิด" แต่ไม่บอกว่าผิดตรงไหน จะถูกข้ามในวันที่คนยุ่ง
> ต้นทุนของการทำเครื่องมือติดตามผลต่ำกว่าต้นทุนของการไล่ด้วยมือซ้ำครั้งที่สาม

## 40. สมมติฐาน "แก้พรอมป์ตแล้ว approval หลุด" — ทดสอบแล้ว **ไม่จริง** (8 ส.ค. 2026)

ทฤษฎีที่ใช้อธิบายว่าทำไม `cowork-task-watchdog` ยิงทุกเช้าแต่ไม่ออกรายงานมา 7 วันคือ
*"การแก้พรอมป์ตทำให้ tool approval ที่เก็บไว้กับ task เป็นโมฆะ รอบถัดไปจึงไปค้างที่หน้าขออนุญาตที่ไม่มีใครกด"*
ฟังดูเข้าเค้ามาก และ **ผิด**

เขียน `tools/task_forensics.py` เอาเวลาที่พรอมป์ตถูกแก้ มาเรียงคู่กับเวลาที่ routine นั้นเขียน runlog ล่าสุด:

| กลุ่ม | ผล |
|---|---|
| แก้พรอมป์ต 1 ส.ค. 17:52 (pantip-monitor · link-health · first-signal · delivery-verify · delivery-heartbeat · comment-loop) | **มี runlog ของ 7 ส.ค. ทุกตัว** |
| เงียบตั้งแต่แก้พรอมป์ต | เหลือ 2 ตัว และทั้งคู่เป็นงานรายสัปดาห์ที่ **ยังไม่ถึงรอบ** (พุธ / จันทร์) |

⇒ แก้พรอมป์ตแล้วงานยังรันต่อได้ปกติ · **สาเหตุที่ watchdog ไม่ออกรายงานยังไม่ทราบ**
สิ่งที่เปลี่ยนไปคือตอนนี้ `agent_gap_check` จับได้เองแล้ว (REPORTING-DARK) ไม่ต้องรอให้คนสังเกต
ขั้นวินิจฉัยถัดไปคือกด Run now ดูว่า "รันตอนมีคนอยู่" ต่างจาก "รันแบบไม่มีคน" หรือไม่

> **บทเรียน: อย่าปล่อยให้ทฤษฎีที่ฟังขึ้นกลายเป็นข้อสรุปโดยไม่ทดสอบ**
> ทฤษฎีนี้เกือบทำให้แนะนำเจ้าของกด Run now สิบกว่างานเพื่อแก้ปัญหาที่ไม่มีอยู่จริง

### 40.1 ของจริงที่เจอระหว่างทดสอบ — ใหญ่กว่าเรื่อง watchdog
**56 งานในหน้าต่าง 9 วัน · มีแค่ 19 ตัวที่สั่งเขียน runlog · อีก 37 ตัวไม่เคยเขียนอะไรเลย**

"ไม่มี runlog" ของ 37 ตัวนั้น **ไม่ใช่หลักฐานว่าพัง** แต่คือ **ไม่มีเครื่องมือวัด**
ฉบับแรกของ `task_forensics.py` พิมพ์ `-` ให้ทั้งสองกรณี ทำให้งานสุขภาพดี 30 กว่าตัวดูเหมือนตายหมด
— **บั๊กคลาสข้อ 31 อีกครั้ง: สองสถานะที่ต่างกันถูกแสดงผลเหมือนกัน** คราวนี้อยู่ในเครื่องมือที่ผมเขียนเองเพื่อไล่บั๊กแบบนี้
แก้แล้วให้แยกคอลัมน์ `logs?` และประกาศตรงๆ ว่ามีกี่ตัวที่ "ตัดสินได้"

> **กฎ: เครื่องมือวินิจฉัยต้องแยก "ไม่มีข้อมูล" ออกจาก "ข้อมูลบอกว่าแย่" ให้ได้ก่อนเสมอ**
> ไม่งั้นมันจะผลิตความมั่นใจผิดๆ ได้เร็วกว่าที่มนุษย์จะตรวจทัน

## 41. ตรวจว่า "ขายได้ไหม" มาตลอด โดยไม่เคยตรวจว่า "ส่งของได้ไหม" (9 ส.ค. 2026)

`site/debt-letter-kit.html` เขียนสัญญาไว้ **3 ครั้ง** ว่า *"ส่งสลิปในแชทเดิม แล้วรับไฟล์ PDF 10 หน้ากลับทางแชท"*
**ไฟล์นั้นไม่มีอยู่จริง** — ทั้งเครื่องมี PDF ไฟล์เดียวคือ ebook 59฿ ซึ่งเป็นคนละสินค้า

หน้านี้เปิดขายและถูกโปรโมตมาหลายสัปดาห์ · ปลายทางรับเงินถูกตรวจถึง **2 รอบ** (25 ก.ค. · 7 ส.ค.) และประกาศว่า "healthy" ทั้งสองครั้ง
เพราะทุกด่านในรีโปนี้ถามว่า **"ข้อเสนอเข้าถึงได้ไหม"** ไม่มีด่านไหนถามว่า **"ของที่สัญญาไว้มีอยู่ไหม"**

> **failure mode ที่แย่ที่สุดที่นี่ ไม่ใช่ "ไม่มีคนซื้อ" แต่คือ "มีคนซื้อแล้วเราส่งของไม่ได้"**
> และมันจะถูกค้นพบโดยลูกค้าคนแรกที่จ่ายเงิน ซึ่งเป็นจังหวะที่แพงที่สุดเท่าที่เป็นไปได้

เพิ่ม `check_deliverables` ใน preflight — สินค้าทุกตัวใน `policy.products` ที่หน้าขายยังมีอยู่ ต้องมีของส่ง
- **FAIL ไม่ใช่ WARN** · preflight FAIL เขียน alert แต่ไม่หยุดสายการโพสต์ จึงดังได้โดยไม่ทำลายอะไร
- ไฟล์เล็กกว่า 10 KB = placeholder ที่ลืมแทน ไม่ใช่สินค้า
- สินค้าที่โฮสต์นอกเครื่อง (Gumroad) **ต้องบอกว่า "ตรวจจากดิสก์ไม่ได้"** ห้ามเดาว่าโอเค
- สินค้าที่หน้าขายถูกลบไปแล้ว = ไม่ใช่สัญญาค้าง ไม่ต้องเตือน
- 11 เคสทดสอบย้อนทาง รวมเคส "ของดี 1 ตัวต้องไม่บังของพัง 1 ตัว"

### 41.1 กับดักที่เกือบทำให้แย่กว่าเดิม
คำแนะนำของผมเมื่อ 8 ส.ค. คือ *"ต่อ `l/debt-toolkit` เข้าหน้าขาย ใช้เวลา 1 ชม."*
แต่ `l/debt-toolkit` ส่งมอบ **สเปรดชีต Excel** ส่วนหน้านั้นขาย **จดหมาย 5 ฉบับ**
**ราคาเท่ากันทั้งคู่คือ 199฿** ซึ่งเป็นเหตุผลเดียวที่ทำให้มองข้ามได้ง่ายขนาดนั้น
⇒ ถ้าทำตามที่ผมสั่ง จะเปลี่ยนจาก **"ขายไม่ได้"** เป็น **"ขายผิดของ"** ซึ่งแย่กว่าเดิมมาก

> **กฎ: ก่อนต่อท่อจ่ายเงินเข้ากับหน้าขาย ต้องยืนยันว่า "ของที่ปลายท่อ" คือ "ของที่หน้านั้นสัญญา"**
> ราคาที่ตรงกันไม่ใช่หลักฐานว่าเป็นสินค้าเดียวกัน — มันคือสิ่งที่ทำให้เข้าใจผิดได้ง่ายที่สุด

### 41.2 กฎที่ตัวเองตั้ง หมดอายุเมื่อสมมติฐานที่รองรับมันผิด
เมื่อ 8 ส.ค. ผมเขียนไว้เองว่า **"ห้ามแตะหน้าขาย ห้ามแก้ราคา ห้ามเขียนใหม่ — n=14 ยังบอกไม่ได้ว่าอะไรผิด"**
กฎนั้นถูกต้อง **ภายใต้สมมติฐานว่าข้อเสนอมีของจริง** พอสมมติฐานพัง กฎก็หมดผลไปด้วย
ถ้ายึดกฎของตัวเองต่อไป จะกลายเป็นปล่อยให้หน้าขายรับเงินโดยส่งของไม่ได้ต่อไปเรื่อยๆ **ในนามของวินัย**

## 42. บั๊กที่ทุกตัวเลขถูกหมด ผิดแค่ว่าอยู่ใต้ชื่อไหน (9 ส.ค. 2026)

รายงานยุทธศาสตร์ 8 ส.ค. ระบุว่า *"มีคนกดปุ่มซื้อ 2 ครั้ง มาจาก Pantip ทั้งคู่"* และใช้เป็นเหตุผลจัดอันดับให้ Pantip มาก่อน
ความจริง: **ทั้ง 2 ครั้งเกิดวันที่ 1 ส.ค. จาก `(direct)` = คลิกทดสอบตอนติดตั้งปุ่มเอง** · **มนุษย์กดปุ่มซื้อ 0 ครั้งตลอดกาล**

สาเหตุใน `ga4_pull.py`: ตัวแปรชื่อช่องถูกคำนวณไว้ใน branch ของ `affiliate_click` เท่านั้น
แถว `buy_intent_click` จึงใช้ค่าที่ **ค้างจากแถวก่อนหน้า** · GA4 เรียงแถวตามจำนวน ทำให้ `pantip affiliate_click 4` มาติดกับ `(direct) buy_intent_click 2` พอดี

> **ทำไมมันรอดสายตามา 8 วัน: ผลรวมทุกตัวถูกหมด ผิดแค่ว่าถูกจัดอยู่ใต้ชื่อไหน**
> ไฟล์ผลลัพธ์ดูสมเหตุสมผลทุกบรรทัด · การรีวิวโค้ดก็มองไม่เห็นเพราะบรรทัดนั้นไม่มีอะไรผิดในตัวมันเอง
> นี่คือบั๊กชนิดที่ **ตรวจด้วยการอ่านไม่เจอ ต้องตรวจด้วยการเล่นลำดับข้อมูลจริงซ้ำเท่านั้น**

เทสต์ที่แก้แล้วต้องพิสูจน์สองทาง — และมันทำจริง: โค้ดเก่าให้ `{'pantip': 2}` (ตรงกับไฟล์ที่ผิดเป๊ะ) โค้ดใหม่ให้ `{'direct': 2}` (ตรงกับ GA4)
มีเคส reverse ด้วยว่า fold ที่ยัดทุกอย่างลง direct ก็ต้องไม่ผ่าน

### 42.1 บทเรียนของฝั่งผู้อ่านรายงาน
ผมเอาเลข 2 นั้นไปเขียนเป็นหลักฐานในเอกสารยุทธศาสตร์ **โดยไม่ได้ถาม GA4 ตรงๆ ว่ามันเกิดวันไหน**
ถ้าถามแค่คำถามเดียวคือ *"2 คลิกนั้นเกิดวันไหน"* จะเจอทันทีว่าอยู่วันเดียวกันคือวันติดตั้งปุ่ม
> **กฎ: ตัวเลข conversion ที่มีค่าน้อยมาก (n ≤ 5) ต้องเปิดดูรายแถวเสมอก่อนเอาไปอ้าง**
> ที่ n เล็กขนาดนั้น "ค่าที่ผิดพลาดจากการทดสอบของเราเอง" กับ "สัญญาณจริง" มีขนาดเท่ากันพอดี
