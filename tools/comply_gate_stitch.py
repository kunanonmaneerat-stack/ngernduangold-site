...
RATE_CTX = re.compile(r'(ดอกเบี้ย|อัตรา|ต่อปี|ผลตอบแทน|ปันผล|APR|interest|rate)\D{0,14}\d', re.I)
...
def main():
    ...
    t=sys.argv[1]
    files=glob.glob(os.path.join(t,'**','*.html'),recursive=True) if os.path.isdir(t) else [t]
    tot=0
    for f in files:
